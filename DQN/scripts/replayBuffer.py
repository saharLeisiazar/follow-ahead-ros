...
class replay_buffer:
    ...
    def add_sample(self, s, a, new_s, r, terminal = 0):
        ...
        if s.shape[0] == 3:
            rollout = {"observation": np.concatenate((s[1,:]-s[0,:], s[2,:2]-s[0,:2]), axis=0),
                        "angle": a,
                        "next_observation": np.concatenate((new_s[1,:]-new_s[0,:], new_s[2,:2]-new_s[0,:2]), axis=0),
                        "reward": r,
                        "terminal": terminal}
        else:
    ...
    def convert_rollouts_to_list(self, rollouts):
        ob_b = np.concatenate([rollout["observation"] for rollout in rollouts]).reshape(len(rollouts), len(rollouts[0]["observation"]))
        action_b = np.array([rollout["angle"] for rollout in rollouts]) 
        next_ob_b = np.concatenate([rollout["next_observation"] for rollout in rollouts]).reshape(len(rollouts), len(rollouts[0]["next_observation"]))
    ...
    def save_to_file(self):
        with open( self.params['save_dir'] + "dataset/"+ self.params['dataset_name'], "wb") as f:   #Pickling
